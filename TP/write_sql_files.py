...
def write_sql_queries(sql_dirpath, sql_queries_d):
    assert os.path.isdir(sql_dirpath), f"El directorio {sql_dirpath} no existe."

    print("Archivos escritos:")
    for fn, sql_query in sql_queries_d.items():
        fp = os.path.abspath(os.path.join(sql_dirpath, fn))
        with open(fp, "w", encoding="utf-8") as f:
            f.write(sql_query.strip())
        print(f'  - "{fp}".')
    print(f"Total: {len(sql_queries_d)}.")
    print()
...
""",
    ...
    write_sql_queries(utils.SQL_DIRPATH_, sql_queries_d)
